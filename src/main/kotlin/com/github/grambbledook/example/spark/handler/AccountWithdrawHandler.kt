...
class AccountWithdrawHandler(private val service: AccountService, override val mapper: ObjectMapper) : AbstractJsonHandler<AccountWithdrawRequest, Receipt<AccountWithdrawDetails>>(mapper, AccountWithdrawRequest::class.java) {
    ...
    override fun process(request: AccountWithdrawRequest): Result {
        ...
        return if (request.amount <= BigDecimal.ZERO) {
            ...
            service.withdraw(request.id, request.amount).map {
